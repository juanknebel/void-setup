...
- **`setup-voidsplash.sh`** = optional boot splash. Writes a self-contained runit service (`/etc/sv/voidsplash/run`) that draws the repo's custom frame `images/void-0.png` (installed to `/etc/voidsplash/`; the official void-logo on a solid black background, 1280x800 for the X220 panel) on the framebuffer via `fbv` until the SDDM greeter (Xorg) is up, then stops the service so it never repaints a TTY after boot. (It draws first and tests after, because runit starts services in parallel and SDDM is often already up on the first check.) **Does NOT use the upstream jaylesworth/voidsplash binary** — that binary loops forever against a hardcoded `nodm` target we don't use, piling up processes that paint over TTYs; our run script targets `sddm` and self-stops instead. Only depends on `fbv`. `--grub-quiet` flag also appends `quiet console=tty2` to `GRUB_CMDLINE_LINUX_DEFAULT` so kernel logs are silenced and any remainder lands on tty2 instead of painting over the splash.
